...
const PARAM_FIELD_META = {
  ...
  'wiping.wipe_travel_enabled': { label: '启用空驶擦动', desc: '开启后在擦料塔上执行空驶擦动操作，有助于清洁喷嘴。', type: 'boolean', group: 'wiping' },
  'wiping.wipe_travel_speed': { label: '空驶擦动速度', desc: '空驶擦动时的移动速度，过快可能导致打印机晃动。', unit: 'mm/s', group: 'wiping' },
  'wiping.tower_base_size': { label: '底面擦料大小', desc: '擦料塔底面的展开大小倍数，1.0 为标准大小。', unit: '倍', group: 'wiping' }
};
...
async function getActivePresetPath() {
  ...
  // 将 .json 后缀改为 .toml
  const tomlFileName = fileName.replace(/\.json$/i, '.toml');
  return `${userDataPath}\\${tomlFileName}`;
}
