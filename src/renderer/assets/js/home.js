...
async function selectBrand(brandId) {
  ...
  refreshHomeSelectionDownstream(currentPrinter);
}
...
function selectPrinter(printerId, keepVersion = false) {
  ...
  saveUserConfig();
  refreshHomeSelectionSurfaces(selectedBrand);
  refreshHomeSelectionDownstream(selectedPrinterObj);
}
...
function refreshHomeSelectionSurfaces(brandId = selectedBrand) {
  renderBrands();
  renderPrinters(brandId);
}

function refreshHomeSelectionDownstream(printer) {
  if (printer && typeof window.renderDownloadVersions === 'function') {
    window.renderDownloadVersions(printer);
  } else if (!printer && typeof window.renderDownloadVersions === 'function') {
    window.renderDownloadVersions(null);
  }
  if (typeof window.refreshCalibrationAvailability === 'function') {
    window.refreshCalibrationAvailability();
...
function refreshEmptyHomeDownstreamSurfaces() {
  if (typeof window.renderDownloadVersions === 'function') {
    window.renderDownloadVersions(null);
  }
  if (typeof window.refreshCalibrationAvailability === 'function') {
...
async function addBrandFlow() {
  ...
  if (await persistCatalogWithFeedback()) {
    selectedBrand = id;
    selectedPrinter = null;
    refreshHomeSelectionSurfaces(id);
    refreshEmptyHomeDownstreamSurfaces();
  }
}
...
async function renameTargetFlow(target) {
  ...
  if (await persistCatalogWithFeedback()) {
    ...
    if (!isBrand && item.id === selectedPrinter) {
      ...
      if (isBrand && item.id === selectedBrand) {
        const sidebarBrand = document.getElementById('sidebarBrand');
        if (sidebarBrand) {
          sidebarBrand.textContent = item.shortName || item.name;
        }
        refreshSelectedBrandDownloadSurface(target);
      }
      saveUserConfig();
  ...
  if (await persistCatalogWithFeedback()) {
    if (selectedPrinter === location.printer.id) {
      ...
      if (fallbackPrinter) {
        ...
        selectedPrinter = null;
        selectedBrand = location.brandId;
        refreshHomeSelectionSurfaces(location.brandId);
        refreshEmptyHomeDownstreamSurfaces();
      }
    } else {
      refreshHomeSelectionSurfaces(selectedBrand);
