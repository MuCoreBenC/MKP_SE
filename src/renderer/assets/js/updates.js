...
function resolveModernUpdateManifest(manifest) {
  const parser = typeof window.__parseUpdateManifestForLegacy__ === 'function'
    ? window.__parseUpdateManifestForLegacy__
    : (typeof window.__MKP_MODERN_BRIDGE__?.parseUpdateManifest === 'function'
      ? (input) => window.__MKP_MODERN_BRIDGE__.parseUpdateManifest(input)
      : null);
  ...
  try {
    return parser(manifest) || manifest;
  } catch (error) {
    if (typeof Logger?.warn === 'function') {
      Logger.warn('应用更新 manifest modern 解析失败，回退 legacy 逻辑', { error: error.message });
    }
    return manifest;
  }
}

function resolveModernUpdateCheck(currentAppVersion, manifest) {
  const checker = typeof window.__checkAppUpdateForLegacy__ === 'function'
    ? window.__checkAppUpdateForLegacy__
    : (typeof window.__MKP_MODERN_BRIDGE__?.checkUpdates === 'function'
      ? (version, input) => window.__MKP_MODERN_BRIDGE__.checkUpdates(version, input)
      : null);
  ...
  try {
    return checker(currentAppVersion, manifest) || null;
  } catch (error) {
    if (typeof Logger?.warn === 'function') {
      Logger.warn('应用更新 modern 检查失败，回退 legacy 逻辑', { error: error.message });
    }
    return null;
  }
}
...
function parseManifestToUI(manifest, currentAppVersion) {
  const resolvedManifest = resolveModernUpdateManifest(manifest);
  const cleanCurrent = normalizeAppVersion(currentAppVersion);
  const uiVersions = [];

  if (!resolvedManifest || typeof resolvedManifest !== 'object' || !resolvedManifest.latestVersion) {
    return uiVersions;
  }

  uiVersions.push({
    version: `v${resolvedManifest.latestVersion}`,
    date: resolvedManifest.releaseDate,
    desc: resolvedManifest.shortDesc || '常规体验优化与错误修复',
    status: resolvedManifest.latestVersion === cleanCurrent ? 'RUNNING' : 'LATEST',
    current: resolvedManifest.latestVersion === cleanCurrent,
    canRollback: resolvedManifest.canRollback !== false,
    details: Array.isArray(resolvedManifest.releaseNotes) ? resolvedManifest.releaseNotes : [],
    downloadUrl: resolvedManifest.downloadUrl || ''
  });

  if (Array.isArray(resolvedManifest.history)) {
    resolvedManifest.history.forEach((item) => {
...
function readModernCachedAppManifest() {
  if (typeof window.__getCachedUpdateManifestView__ !== 'function') {
    return null;
  }

  return window.__getCachedUpdateManifestView__() || null;
}

function persistAppUpdateState(payload = {}) {
  const state = {
    currentVersion: normalizeAppVersion(payload.currentVersion || APP_REAL_VERSION),
    latestVersion: normalizeAppVersion(payload.latestVersion),
    hasUpdate: !!payload.hasUpdate,
    checkedAt: Date.now()
  };

  const modernState = readModernAppUpdateState();
  if (modernState) {
    return state;
  }
...
function readModernAppUpdateState() {
  const view = typeof window.__getAppUpdateStateView__ === 'function'
    ? window.__getAppUpdateStateView__()
    : null;
  ...
  return {
    currentVersion: normalizeAppVersion(view.currentVersion || APP_REAL_VERSION),
    latestVersion: normalizeAppVersion(view.latestVersion),
    hasUpdate: !!view.hasUpdate,
    checkedAt: view.checkedAt ? new Date(view.checkedAt).getTime() : null
  };
}

function readModernUpdateMode() {
  if (typeof window.__getUpdateModeView__ !== 'function') {
    return null;
  }

  const mode = window.__getUpdateModeView__();
...
function readAppUpdateState() {
  ...
  return readLegacyAppUpdateState();
}

function readLegacyAppUpdateState() {
  try {
...
function syncAppUpdateState(manifest, currentAppVersion) {
  const resolvedManifest = resolveModernUpdateManifest(manifest);
  if (!resolvedManifest?.latestVersion) return false;

  const modernCheck = resolveModernUpdateCheck(currentAppVersion, resolvedManifest);
  const cleanCurrent = normalizeAppVersion(modernCheck?.currentVersion || currentAppVersion);
  const latestVersion = normalizeAppVersion(modernCheck?.latestVersion || resolvedManifest.latestVersion);
  const hasUpdate = typeof modernCheck?.hasUpdate === 'boolean'
    ? modernCheck.hasUpdate
    : compareVersionsFront(latestVersion, cleanCurrent) > 0;

  persistAppUpdateState({ currentVersion: cleanCurrent, latestVersion, hasUpdate });
...
function hydrateCachedAppUpdateBadge() {
  const modernState = readModernAppUpdateState();
  if (modernState) {
    applyAppUpdateBadge(!!modernState.hasUpdate);
    return;
  }

  const cached = readLegacyAppUpdateState();
  if (!cached?.latestVersion) return;

  const currentVersion = normalizeAppVersion(cached.currentVersion || APP_REAL_VERSION);
  const hasUpdate = compareVersionsFront(cached.latestVersion, currentVersion) > 0;
  applyAppUpdateBadge(hasUpdate);
}
...
async function loadLocalManifest() {
  try {
    ...
    const localManifest = readModernCachedAppManifest() || await readLocalAppManifest();
...
async function silentCheckForUpdate(mode, options = {}) {
  ...
  try {
    ...
    const resolvedManifest = resolveModernUpdateManifest(manifest) || manifest;
    await saveCloudManifestToLocal(resolvedManifest);

    const currentVersion = await getCurrentAppVersion();
    syncAppUpdateState(resolvedManifest, currentVersion);
    globalVersions = parseManifestToUI(resolvedManifest, currentVersion);
    renderVersions();
  } catch (error) {
...
const APP_UPDATE_MODE_INIT_KEY = 'update_mode_initialized_v2';
let appUpdatePanelHideTimer = null;
...
function ensureAppUpdatePanel() {
  ...
  panel.className = 'pointer-events-auto fixed right-6 top-6 z-[10060] hidden w-[380px] max-w-[calc(100vw-32px)] translate-y-[-12px] scale-[0.98] rounded-2xl border border-gray-200 bg-white/96 p-4 opacity-0 shadow-[0_24px_60px_rgba(15,23,42,0.18)] backdrop-blur transition-all duration-300 ease-out dark:border-[#333] dark:bg-[#1f1f20]/96';
  panel.innerHTML = `
...
async function silentCheckForUpdate(mode, options = {}) {
  ...
  const modernState = readModernAppUpdateState();
  const cachedState = modernState || readLegacyAppUpdateState();
  if (modernState) {
    applyAppUpdateBadge(!!modernState.hasUpdate);
  } else if (cachedState?.hasUpdate) {
    const currentVersion = normalizeAppVersion(cachedState.currentVersion || APP_REAL_VERSION);
    applyAppUpdateBadge(compareVersionsFront(cachedState.latestVersion, currentVersion) > 0);
  }
...
const APP_UPDATE_PANEL_V2_ID = 'mkp-app-update-panel';
const APP_UPDATE_MODE_INIT_V2_KEY = 'update_mode_initialized_v2';
let appUpdatePanelV2HideTimer = null;

function ensureAppUpdatePanel() {
  let panel = document.getElementById(APP_UPDATE_PANEL_V2_ID);
  if (panel) {
    return panel;
  }

  panel = document.createElement('div');
  panel.id = APP_UPDATE_PANEL_V2_ID;
  panel.className = 'pointer-events-auto fixed right-6 top-6 z-[10060] hidden w-[380px] max-w-[calc(100vw-32px)] translate-y-[-12px] scale-[0.98] rounded-2xl border border-gray-200 bg-white/96 p-4 opacity-0 shadow-[0_24px_60px_rgba(15,23,42,0.18)] backdrop-blur transition-all duration-300 ease-out dark:border-[#333] dark:bg-[#1f1f20]/96';
  panel.innerHTML = `
    <div class="flex items-start gap-3">
      <div id="mkp-app-update-panel-icon-v2" class="mt-0.5 flex h-10 w-10 items-center justify-center rounded-2xl bg-blue-50 text-blue-600 dark:bg-blue-500/12 dark:text-blue-300">
        <svg class="h-5 w-5 animate-spin" fill="none" viewBox="0 0 24 24">
          <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
          <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
        </svg>
      ...
      <div class="min-w-0 flex-1">
        <div id="mkp-app-update-panel-title-v2" class="text-sm font-bold text-gray-900 dark:text-gray-100">正在检查更新</div>
        <div id="mkp-app-update-panel-desc-v2" class="mt-1 text-xs leading-5 text-gray-500 dark:text-gray-400">正在连接云端版本清单...</div>
        <div id="mkp-app-update-panel-meta-v2" class="mt-2 text-[11px] font-medium tracking-[0.01em] text-gray-400 dark:text-gray-500"></div>
        <div class="mt-3 h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-white/8">
          <div id="mkp-app-update-panel-bar-v2" class="h-full w-[16%] rounded-full bg-blue-500 transition-all duration-500"></div>
        </div>
      </div>
      <button id="mkp-app-update-panel-close-v2" type="button" class="ml-1 inline-flex h-8 w-8 items-center justify-center rounded-xl text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-white/8 dark:hover:text-gray-200">
        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
        </svg>
      </button>
  ...
  document.body.appendChild(panel);
  panel.querySelector('#mkp-app-update-panel-close-v2')?.addEventListener('click', () => {
    hideAppUpdatePanel();
  });
  return panel;
}
...
function setAppUpdatePanelState(options = {}) {
  const panel = ensureAppUpdatePanel();
  const iconWrap = panel.querySelector('#mkp-app-update-panel-icon-v2');
  const title = panel.querySelector('#mkp-app-update-panel-title-v2');
  const desc = panel.querySelector('#mkp-app-update-panel-desc-v2');
  const meta = panel.querySelector('#mkp-app-update-panel-meta-v2');
  const bar = panel.querySelector('#mkp-app-update-panel-bar-v2');
  if (!iconWrap || !title || !desc || !meta || !bar) {
    return;
  }

  if (appUpdatePanelV2HideTimer) {
    window.clearTimeout(appUpdatePanelV2HideTimer);
    appUpdatePanelV2HideTimer = null;
  }
  ...
  window.requestAnimationFrame(() => {
    panel.classList.remove('translate-y-[-12px]', 'scale-[0.98]', 'opacity-0');
    panel.classList.add('translate-y-0', 'scale-100', 'opacity-100');
  });
}

function hideAppUpdatePanel(delayMs = 0) {
  const panel = document.getElementById(APP_UPDATE_PANEL_V2_ID);
  if (!panel) {
    return;
  }
  ...
  const close = () => {
    ...
    panel.classList.add('translate-y-[-12px]', 'scale-[0.98]', 'opacity-0');
    appUpdatePanelV2HideTimer = window.setTimeout(() => {
      panel.classList.add('hidden');
      appUpdatePanelV2HideTimer = null;
    }, 220);
  };
  ...
  if (delayMs > 0) {
    ...
    appUpdatePanelV2HideTimer = window.setTimeout(close, delayMs);
    return;
  }
...
function saveUpdateMode(mode) {
  Logger.info(`Write variable: update_mode, v:${mode}`);
  if (typeof window.__setUpdateModeForLegacy__ === 'function') {
    window.__setUpdateModeForLegacy__(mode);
    silentCheckForUpdate(mode, { force: true });
    return;
  }
  localStorage.setItem(APP_UPDATE_MODE_KEY, mode);
  localStorage.setItem(APP_UPDATE_MODE_INIT_V2_KEY, 'true');
  silentCheckForUpdate(mode, { force: true });
}

function initUpdateModeSetting() {
  Logger.info('Read variable: update_mode');
  const modernMode = readModernUpdateMode();
  let savedMode = modernMode;
  if (modernMode) {
    savedMode = modernMode;
  } else {
    savedMode = localStorage.getItem(APP_UPDATE_MODE_KEY);
    const hasInitialized = localStorage.getItem(APP_UPDATE_MODE_INIT_V2_KEY) === 'true';
    if (!savedMode || !hasInitialized) {
      savedMode = 'manual';
      if (typeof window.__setUpdateModeForLegacy__ === 'function') {
        window.__setUpdateModeForLegacy__(savedMode);
      }
      localStorage.setItem(APP_UPDATE_MODE_KEY, savedMode);
      localStorage.setItem(APP_UPDATE_MODE_INIT_V2_KEY, 'true');
    }
...
async function manualCheckAppUpdate(btnElement) {
  ...
  try {
    const remoteManifest = await fetchAppManifestWithFallback();
    const resolvedManifest = resolveModernUpdateManifest(remoteManifest) || remoteManifest;
    await saveCloudManifestToLocal(resolvedManifest);

    const currentAppVersion = await getCurrentAppVersion();
    const remoteVersion = normalizeAppVersion(resolvedManifest.latestVersion);
    setAppUpdatePanelState({
      title: '已获取云端版本',
      desc: `检测到远端版本 v${remoteVersion}`,
      meta: `当前 v${currentAppVersion} -> 远端 v${remoteVersion}`,
      progress: 38,
      tone: 'info'
    });

    globalVersions = parseManifestToUI(resolvedManifest, currentAppVersion);
    renderVersions();

    const hasUpdate = syncAppUpdateState(resolvedManifest, currentAppVersion);
    if (!hasUpdate) {
      if (resetEngine) {
        resetEngine();
      }
      setAppUpdatePanelState({
        title: '已是最新版本',
        desc: `当前运行的 v${currentAppVersion} 已经是最新版本。`,
        meta: `当前 v${currentAppVersion} 与远端一致`,
        progress: 100,
        tone: 'success'
      });
      hideAppUpdatePanel(1800);
    ...
    const confirmed = await MKPModal.confirm({
      ...
      msg: `${resolvedManifest.shortDesc || '常规体验优化与错误修复'}\n\n是否立即下载并应用热更新？`,
      type: 'info',
      confirmText: '立即更新',
      cancelText: '稍后再说'
    });
    ...
    if (!confirmed) {
      setAppUpdatePanelState({
        title: '发现新版本',
        desc: `已保留 v${remoteVersion} 提醒，左侧版本控制会显示红点。`,
        meta: `当前 v${currentAppVersion} -> 新版 v${remoteVersion}`,
        progress: 100,
        tone: 'success'
      });
      hideAppUpdatePanel(1800);
    ...
    setAppUpdatePanelState({
      title: '正在下载更新',
      desc: '补丁包下载中，请稍候...',
      meta: `当前 v${currentAppVersion} -> 目标 v${remoteVersion}`,
      progress: 62,
      tone: 'info'
    });

    const updateResult = await window.mkpAPI.applyHotUpdate({
      urls: buildPatchUrlCandidates(resolvedManifest.downloadUrl),
    ...
    setAppUpdatePanelState({
      title: '正在应用更新',
      desc: `补丁已写入 v${updateResult.version || remoteVersion}，即将重启软件。`,
      meta: '热更新补丁已应用，正在准备重启',
      progress: 100,
      tone: 'success'
    });
