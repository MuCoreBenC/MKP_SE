...
function loadUserConfig() {
  try {
    ...
    if (saved) {
      ...
      if (typeof window.__hydrateModernContextFromLegacy__ === 'function') {
        window.__hydrateModernContextFromLegacy__();
      }
    }
...
function resolvePersistedVersionForPrinter(printerId, preferredVersion = null, config = {}) {
  ...
  if (typeof window.__getActivePresetView__ === 'function' && window.__getActivePresetView__()?.fileName && preferredVersion) {
    return supportedVersions.includes(preferredVersion) ? preferredVersion : supportedVersions[0] || null;
  }
...
function hasRestorablePresetSession() {
  ...
  const legacyActiveFileName = localStorage.getItem(`mkp_current_script_${selectedPrinter}_${restoredVersion}`);
  const activeFileName = typeof window.__getActivePresetView__ === 'function'
    ? window.__getActivePresetView__()?.fileName || legacyActiveFileName
    : legacyActiveFileName;
  return !!activeFileName;
}

function getResolvedActiveFileName(currentKey) {
  const legacyActiveFileName = localStorage.getItem(`mkp_current_script_${currentKey}`);
  ...
  try {
    ...
    if (result.success) {
      ...
      const modernView = typeof window.__getDownloadContextView__ === 'function'
        ...
        : null;
      const printerData = modernView?.printer || getPrinterObj(selectedPrinter);
  ...
  try {
    ...
    try {
        if (window.mkpAPI && window.mkpAPI.saveLocalPresetsManifest) {
            ...
            const printerData = modernView?.printer || getPrinterObj(selectedPrinter);
            const versionType = modernView?.selectedVersionType || selectedVersion;
            if (printerData) renderPresetList(printerData, versionType);
        }
    } catch (saveErr) {
...
async function checkOnlineUpdates(btnElement) {
  ...
  try {
    const [cloudResult] = await Promise.all([
      fetchCloudPresets(printerData.id, versionType),
    ...
    renderListItems(onlineList, releases, printerData, versionType, false);
...
function renderDownloadVersions(printerData) {
  ...
  renderVersionCards('downloadVersionList', printerData, selectedVersion, (vType) => {
    ...
    const activeFileName = getResolvedActiveFileName(currentKey);
...
async function renderPresetList(printerData, versionType) {
  ...
  if (localData.length > 0) {
    ...
    // 判断是否有选中的配置
    const activeFileName = getResolvedActiveFileName(`${printerData.id}_${versionType}`);
...
function renderListItems(container, releases, printerData, versionType, isLocal) {
  ...
  releases.forEach((release) => {
    const activeFileName = getResolvedActiveFileName(`${printerData.id}_${versionType}`);
    const isApplied = isLocal && (release.fileName === activeFileName);
...
async function updateScriptPathDisplay() {
  ...
  try {
    const legacyPresetPath = await getActivePresetPath();
    const presetPath = typeof window.__getParamsPresetView__ === 'function'
      ? window.__getParamsPresetView__()?.absolutePath || legacyPresetPath
      : legacyPresetPath;
...
async function getCalibrationAvailability() {
  const modernView = typeof window.__getCalibrationContextView__ === 'function'
    ? window.__getCalibrationContextView__()
    : null;
  const resolvedPrinterId = modernView?.printerId || selectedPrinter;
  const resolvedVersionType = modernView?.versionType || selectedVersion;
  const activePresetPath = (typeof window.__getCalibrationContextView__ === 'function'
    ? window.__getCalibrationContextView__()?.presetPath
    : null) || (typeof getActivePresetPath === 'function'
