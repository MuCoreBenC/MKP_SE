var MKPReactPagesBundle = (function(exports) {
	...
	*/
	...
	//#endregion
	//#region src/renderer/react-app/runtime/react-page-runtime.tsx
	var import_react = /* @__PURE__ */ __toESM(require_react());
	var import_client = require_client();
	var import_jsx_runtime = require_jsx_runtime();
	var ReactPageErrorBoundary = class extends import_react.Component {
		constructor(props) {
			super(props);
	...
	function ReadySignal({ onReady, children }) {
	...
	function setReactPageVisibility(target, fallbackTarget, reactVisible) {
	...
	function LegacyShellBridge({ fallbackTarget, onReady }) {
		if (!fallbackTarget) throw new Error("LegacyShellBridge requires a fallback target");
		const hostRef = (0, import_react.useRef)(null);
		const adoptedNodesRef = (0, import_react.useRef)([]);
		(0, import_react.useLayoutEffect)(() => {
		...
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			ref: hostRef,
			className: "mkp-react-page-legacy-bridge"
		});
	}
	function createReactPageMount({ pageName, render }) {
		let root = null;
		return function mountReactPage({ target, fallbackTarget }) {
			...
			if (!root) root = (0, import_client.createRoot)(target);
			try {
				root.render(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_react.StrictMode, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ReactPageErrorBoundary, {
					onError: (error) => {
	...
	//#endregion
	//#region src/renderer/react-app/stores/useDownloadPageUiStore.ts
	var useDownloadPageUiStore = create((set) => ({
		localSearchQuery: "",
	...
	//#endregion
	//#region src/renderer/react-app/hooks/useDownloadContext.ts
	var DOWNLOAD_CONTEXT_UPDATED_EVENT = "mkp:download-context-updated";
	var FALLBACK_VIEW = {
		printer: null,
		selectedVersionType: null
	};
	function resolvePrinterFromLegacyState(selectedPrinterId) {
		if (!selectedPrinterId || typeof window === "undefined" || !window.printersByBrand) return null;
	...
	function getSnapshot() {
		if (typeof window === "undefined") return FALLBACK_VIEW;
		const runtimeView = typeof window.__getDownloadContextView__ === "function" ? window.__getDownloadContextView__() : null;
		...
		const selectedPrinterId = hasLegacyPrinterSelection ? window.selectedPrinter ?? null : runtimeView?.printer?.id ?? null;
		const selectedVersionType = hasLegacyVersionSelection ? window.selectedVersion ?? null : runtimeView?.selectedVersionType ?? null;
		return {
			printer: resolvePrinterFromLegacyState(selectedPrinterId) ?? runtimeView?.printer ?? null,
			selectedVersionType
		};
	}
	...
	function useDownloadContext() {
		return (0, import_react.useSyncExternalStore)(subscribe, getSnapshot, () => FALLBACK_VIEW);
	}
	//#endregion
	//#region src/renderer/react-app/hooks/usePresetActions.ts
	var CLOUD_BASES$1 = {
	...
	function buildPresetDownloadUrls(fileName) {
		return [
			`${CLOUD_BASES$1.gitee}/cloud_data/presets/${fileName}`,
			`${CLOUD_BASES$1.jsDelivr}/cloud_data/presets/${fileName}`,
			`${CLOUD_BASES$1.github}/cloud_data/presets/${fileName}`
		];
	}
	function getPinnedPresetKey$1(printerId, versionType) {
	...
	function usePresetActions() {
		...
		return {
			applyLocalPreset: (0, import_react.useCallback)((fileName) => {
				if (!context.printer?.id || !context.selectedVersionType) return {
					success: false,
					error: "missing preset context"
				};
			...
			duplicateLocalPreset: (0, import_react.useCallback)(async ({ fileName, realVersion }) => {
				if (!context.printer?.id || !window.mkpAPI?.duplicatePreset) return {
					success: false,
					error: "duplicatePreset unavailable"
				};
				...
				context.printer?.id,
				context.selectedVersionType,
				setNewlyDownloadedFile
			]),
			renameLocalPreset: (0, import_react.useCallback)(async ({ fileName, newName }) => {
				if (!window.mkpAPI?.renamePresetDisplay) return {
					success: false,
					error: "renamePresetDisplay unavailable"
				};
				const result = await window.mkpAPI.renamePresetDisplay({
					fileName,
					newName
				});
			...
			deleteLocalPreset: (0, import_react.useCallback)(async (fileName) => {
				if (!window.mkpAPI?.deleteFile) return {
					success: false,
					error: "deleteFile unavailable"
				};
				const result = await window.mkpAPI.deleteFile(fileName);
				if (result.success) bumpDataRevision();
				return result;
			}, [bumpDataRevision]),
			downloadOnlinePreset: (0, import_react.useCallback)(async ({ fileName }) => {
				if (!window.mkpAPI?.downloadFile && !window.mkpAPI?.copyBundledPreset) return {
					success: false,
					error: "download APIs unavailable"
				};
				let result = {
					success: false,
					error: "all download nodes failed"
				};
				if (window.mkpAPI?.downloadFile) for (const url of buildPresetDownloadUrls(fileName)) try {
				...
				if (!result.success && window.mkpAPI?.copyBundledPreset) result = await window.mkpAPI.copyBundledPreset(fileName);
				if (result.success) {
			...
			togglePinnedPreset: (0, import_react.useCallback)((fileName) => {
				if (!context.printer?.id || !context.selectedVersionType) return false;
				const storageKey = getPinnedPresetKey$1(context.printer.id, context.selectedVersionType);
				let pinnedFiles = [];
				try {
				...
				if (nextSet.has(fileName)) nextSet.delete(fileName);
				else nextSet.add(fileName);
			...
			refreshOnlinePresets: (0, import_react.useCallback)(async () => {
				bumpDataRevision();
				return { success: true };
			}, [bumpDataRevision])
		};
	...
	//#endregion
	//#region src/renderer/react-app/pages/download/components/LocalPresetList.tsx
	...
	function LocalPresetList({ printerName, selectedVersion, items, loading }) {
		...
		const [pendingFileName, setPendingFileName] = (0, import_react.useState)(null);
		const [pendingDuplicateFileName, setPendingDuplicateFileName] = (0, import_react.useState)(null);
		const [pendingDeleteFileName, setPendingDeleteFileName] = (0, import_react.useState)(null);
		...
		function handleInvertSelection() {
			...
			setSelectedLocalFiles(items.map((item) => item.fileName).filter((fileName) => !currentSet.has(fileName)));
		}
		async function handleBatchDelete() {
			for (const fileName of selectedLocalFiles) await deleteLocalPreset(fileName);
			setSelectedLocalFiles([]);
			setMultiSelectMode(false);
		}
		function handleApplyClick(fileName) {
		...
		async function handleDuplicateClick(fileName, realVersion) {
			setPendingDuplicateFileName(fileName);
			try {
				await duplicateLocalPreset({
					fileName,
					realVersion
				});
			} finally {
		...
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
			className: "rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-[#333] dark:bg-[#252526]",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						...
						children: "已存放在您电脑中的配置，可搜索、排序、多选和右键管理。"
					}),
					...
					children: [
						isMultiSelectMode ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
								className: "rounded-full bg-gray-100 px-3 py-1 text-xs text-gray-500 dark:bg-[#1E1E1E] dark:text-gray-400",
								children: ["已选 ", selectedLocalFiles.length]
							}),
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("select", {
							value: localSortMode,
							onChange: (event) => setLocalSortMode(event.target.value),
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							type: "button",
							onClick: () => {
								if (isMultiSelectMode) setSelectedLocalFiles([]);
								setMultiSelectMode(!isMultiSelectMode);
							},
							className: `inline-flex h-10 items-center justify-center rounded-xl px-4 text-sm font-medium transition-all ${isMultiSelectMode ? "bg-[rgba(var(--primary-rgb),0.12)] text-[rgb(var(--primary-rgb))]" : "border border-gray-200 text-gray-600 hover:bg-gray-100 dark:border-[#444] dark:text-gray-300 dark:hover:bg-[#2A2A2A]"}`,
							children: isMultiSelectMode ? "退出多选" : "批量管理"
						})
					]
				})]
				...
				children: loading ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "rounded-2xl border border-dashed border-gray-200 bg-gray-50/70 p-4 text-sm text-gray-500 dark:border-[#333] dark:bg-[#1E1E1E] dark:text-gray-400",
					children: "正在读取本地预设列表..."
				}) : items.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "rounded-2xl border border-dashed border-gray-200 bg-gray-50/70 p-4 text-sm text-gray-500 dark:border-[#333] dark:bg-[#1E1E1E] dark:text-gray-400",
					children: "当前上下文下没有可显示的本地预设。"
				}) : items.map((item) => {
					const isSelected = selectedLocalFiles.includes(item.fileName);
					return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						onClick: isMultiSelectMode ? () => toggleSelectedFile(item.fileName) : void 0,
						className: `rounded-2xl border p-4 transition-all ${isMultiSelectMode && isSelected ? "border-[rgb(var(--primary-rgb))] bg-[rgba(var(--primary-rgb),0.08)]" : item.isApplied ? "border-[rgb(var(--primary-rgb))] bg-[rgba(var(--primary-rgb),0.08)] dark:bg-[rgba(var(--primary-rgb),0.16)]" : "border-gray-200 bg-gray-50/60 dark:border-[#333] dark:bg-[#1E1E1E]"}`,
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
									...
									onClick: (event) => {
										...
										if (isMultiSelectMode) toggleSelectedFile(item.fileName);
										else togglePinnedPreset(item.fileName);
									},
									className: `mt-0.5 inline-flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full transition-all ${isMultiSelectMode ? isSelected ? "bg-[rgb(var(--primary-rgb))] text-white" : "border border-gray-300 text-gray-400 dark:border-gray-600 dark:text-gray-500" : item.isPinned ? "bg-[rgba(var(--primary-rgb),0.12)] text-[rgb(var(--primary-rgb))]" : "text-gray-300 hover:bg-gray-100 hover:text-gray-500 dark:text-gray-600 dark:hover:bg-[#252526] dark:hover:text-gray-300"}`,
									"aria-label": isMultiSelectMode ? "切换选择" : item.isPinned ? "取消置顶" : "置顶",
									title: isMultiSelectMode ? "切换选择" : item.isPinned ? "取消置顶" : "置顶",
									children: isMultiSelectMode ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
										className: "h-4 w-4",
										fill: "none",
										stroke: "currentColor",
										viewBox: "0 0 24 24",
										...
										children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
											...
											strokeLinejoin: "round",
											strokeWidth: "1.8",
											d: "M6.75 3.75h10.5v16.5L12 17.25 6.75 20.25V3.75z"
										})
									})
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										...
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
											...
											children: item.displayTitle
										}), item.isLatest ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
											className: "rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-medium text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300",
											children: "最新"
										}) : null]
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										className: "mt-1 flex flex-wrap items-center gap-2 text-xs text-gray-500 dark:text-gray-400",
										children: [
											/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
												className: "font-mono",
												children: item.fileName
											}),
											/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: ["v", item.realVersion] }),
											/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: formatDate(item.modifiedAt) }),
											/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: formatSize(item.size) })
										]
									})]
								})]
							}), !isMultiSelectMode ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
										...
										children: pendingDuplicateFileName === item.fileName ? "复制中..." : "复制"
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
										type: "button",
										...
										children: pendingDeleteFileName === item.fileName ? "删除中..." : "删除"
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
										type: "button",
										onClick: () => handleApplyClick(item.fileName),
										disabled: item.isApplied || pendingFileName === item.fileName,
										className: `inline-flex h-10 min-w-[88px] items-center justify-center rounded-xl px-4 text-sm font-medium transition-all ${item.isApplied ? "bg-[rgb(var(--primary-rgb))] text-white" : "theme-btn-soft"} disabled:cursor-default disabled:opacity-100`,
										children: item.isApplied ? "已应用" : pendingFileName === item.fileName ? "应用中..." : "应用"
									})
								]
							}) : null]
						})
	...
	//#endregion
	//#region src/renderer/react-app/hooks/useOnlinePresetList.ts
	var CLOUD_BASES = {
		gitee: "https://gitee.com/MuCoreBenC/MKP_Support_Electron/raw/main",
		jsDelivr: "https://cdn.jsdelivr.net/gh/MuCoreBenC/MKP_Support_Electron@main",
		github: "https://raw.githubusercontent.com/MuCoreBenC/MKP_Support_Electron/main"
	};
	function compareVersionsDesc$1(left, right) {
		if (typeof window.compareVersionsFront === "function") return window.compareVersionsFront(right, left);
		return right.localeCompare(left, void 0, {
			numeric: true,
			sensitivity: "base"
		});
	}
	function inferPresetType(preset) {
		const directType = String(preset.type || "").trim().toLowerCase();
		if (directType) return directType;
		const segments = String(preset.file || "").toLowerCase().replace(/\.json$/i, "").split("_");
	...
	async function readRemoteManifest() {
		...
		for (const url of manifestUrls) try {
			...
			if (manifest && Array.isArray(manifest.presets)) return manifest;
		} catch (error) {
			continue;
		}
		return null;
	}
	function useOnlinePresetList() {
		const context = useDownloadContext();
		...
		(0, import_react.useEffect)(() => {
			...
			async function run() {
				...
				const items = (manifestData?.presets || []).filter((preset) => String(preset.id || "").trim().toLowerCase() === requestedPrinterId).filter((preset) => {
				...
				}).sort((left, right) => compareVersionsDesc$1(left.version || "0.0.0", right.version || "0.0.0")).map((preset, index) => ({
					fileName: String(preset.file || ""),
					displayTitle: [String(preset.file || "").replace(/\.json$/i, ""), preset.version ? `v${preset.version}` : ""].filter(Boolean).join(" "),
	...
	//#endregion
	//#region src/renderer/react-app/pages/download/components/OnlinePresetList.tsx
	function OnlinePresetList({ selectedVersion }) {
		...
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "mt-1 text-xs text-gray-500 dark:text-gray-400",
						children: "云端最新的配置文件，下载后将出现在本地预设中。"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "mt-2 text-[11px] text-gray-400 dark:text-gray-500",
						children: ["当前版本上下文：", selectedVersion || "未选择"]
					})
				] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
					type: "button",
					...
					children: refreshing ? "检查中..." : "检查预设"
				})]
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "space-y-3",
				children: loading ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "rounded-2xl border border-dashed border-gray-200 bg-gray-50/70 px-4 py-5 text-sm text-gray-500 dark:border-[#333] dark:bg-[#1E1E1E] dark:text-gray-400",
					children: "正在读取在线预设列表..."
				}) : items.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "rounded-2xl border border-dashed border-gray-200 bg-gray-50/70 px-4 py-5 text-sm text-gray-500 dark:border-[#333] dark:bg-[#1E1E1E] dark:text-gray-400",
					children: "当前版本上下文下没有可显示的在线预设。"
					...
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: pendingFileName === item.fileName ? "下载中..." : "下载"
						})]
					})
				}, item.fileName))
			})]
	...
	//#endregion
	//#region src/renderer/react-app/pages/download/components/VersionSelector.tsx
	var VERSION_LABELS = {
		standard: "标准版",
		quick: "快速版",
		lite: "Lite 版"
	};
	var VERSION_DESCRIPTIONS = {
		standard: "适合新手和稳定优先场景，保留完整支撑面流程。",
		quick: "适合已经熟悉装配步骤时的快速使用场景。",
		lite: "轻量配置，保留基础能力，适合测试和极简流程。"
	};
	function VersionSelector({ versions, selectedVersion, pendingVersion, onSelectVersion }) {
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
			className: "space-y-4",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "flex items-center gap-3",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-gray-400 shadow-sm ring-1 ring-gray-200 dark:bg-[#252526] dark:ring-[#333]",
					...
					children: "先确定当前机型对应的版本，再管理本地和在线预设。"
				})] })]
			}), versions.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "rounded-[26px] border border-dashed border-gray-200 bg-gray-50/80 p-5 text-sm text-gray-500 dark:border-[#333] dark:bg-[#1E1E1E] dark:text-gray-400",
				children: "请先在“选择机型”页完成机型选择，再返回这里挑选版本。"
			}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				children: versions.map((version) => {
					...
					return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						...
						className: `cursor-pointer rounded-[26px] border p-5 transition-all ${active ? "border-[rgb(var(--primary-rgb))] bg-[rgba(var(--primary-rgb),0.08)] shadow-[0_20px_45px_rgba(var(--primary-rgb),0.12)] dark:bg-[rgba(var(--primary-rgb),0.18)]" : "border-gray-200 bg-white hover:border-gray-300 dark:border-[#333] dark:bg-[#252526]"} ${pending ? "opacity-70" : ""}`,
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "flex items-center justify-between gap-4",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "min-w-0",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									...
									children: VERSION_LABELS[version] || version
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: "mt-1 text-sm text-gray-500 dark:text-gray-400",
									children: VERSION_DESCRIPTIONS[version] || `版本键：${version}`
								})]
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: `inline-flex h-8 min-w-[72px] items-center justify-center rounded-full px-3 text-xs font-medium ${active ? "bg-[rgb(var(--primary-rgb))] text-white" : "bg-gray-100 text-gray-500 dark:bg-[#1E1E1E] dark:text-gray-400"}`,
								children: pending ? "切换中" : active ? "当前选择" : "可选"
							})]
						})
					}, version);
				})
	...
	//#endregion
	//#region src/renderer/react-app/hooks/useLocalPresetList.ts
	function getResolvedActiveFileName(printerId, versionType) {
		...
		return typeof window.resolveActivePresetFileName === "function" ? window.resolveActivePresetFileName(window, legacyActiveFileName) : legacyActiveFileName;
	}
	...
	function useLocalPresetList() {
		...
		(0, import_react.useEffect)(() => {
			...
			async function run() {
				...
				let items = (result?.success ? result.data ?? [] : []).filter((item) => {
					const normalizedFileName = String(item.fileName || "").toLowerCase();
					if (normalizedFileName.startsWith(prefix)) return true;
					return String(item.presetType || "").trim().toLowerCase() === String(versionType || "").trim().toLowerCase() && malformedPrefixes.some((badPrefix) => normalizedFileName.startsWith(badPrefix));
				}).map((item) => ({
				...
				if (localSearchQuery) items = items.filter((item) => {
					return item.displayTitle.toLowerCase().includes(localSearchQuery) || item.fileName.toLowerCase().includes(localSearchQuery) || item.realVersion.toLowerCase().includes(localSearchQuery);
				});
				...
				items.sort((left, right) => {
					...
					if (localSortMode === "updated-desc") return (right.modifiedAt || 0) - (left.modifiedAt || 0);
					if (localSortMode === "name-asc") return left.displayTitle.localeCompare(right.displayTitle, "zh-CN", {
						numeric: true,
						sensitivity: "base"
					});
	...
	//#endregion
	//#region src/renderer/react-app/hooks/useVersionSelection.ts
	function useVersionSelection() {
		...
		return {
			...
			selectVersion: (0, import_react.useCallback)((versionType) => {
				if (versionType && !context.printer?.id) return;
				setPending(versionType);
				resetTransientState();
				if (typeof window !== "undefined") {
					if (typeof window.selectedVersion !== "undefined") window.selectedVersion = versionType;
					if (versionType && typeof window.__syncLegacyContextToModern__ === "function" && context.printer?.id) window.__syncLegacyContextToModern__({
						printerId: context.printer.id,
						versionType
					});
					if (typeof window.updateSidebarVersionBadge === "function") window.updateSidebarVersionBadge(versionType);
					if (typeof window.saveUserConfig === "function") window.saveUserConfig();
	...
	//#endregion
	//#region src/renderer/react-app/pages/download/DownloadPage.tsx
	function DownloadPage() {
		const context = useDownloadContext();
		const versions = context.printer?.supportedVersions || [];
		...
		const hintText = !context.printer?.id ? "请先选择机型" : !context.selectedVersionType ? "请先选择版本类型" : !hasLocalPreset ? "请先准备本地预设" : "";
		function navigateToPage(page) {
			if (typeof document === "undefined") return;
			document.querySelector(`[data-page='${page}']`)?.click();
		}
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "page-header page-header-fixed-shell w-full",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "page-header-top",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
					id: "title-page-download",
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
							...
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "上一步" })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
							type: "button",
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "下一步" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
								className: "h-4 w-4",
								fill: "none",
								stroke: "currentColor",
								viewBox: "0 0 24 24",
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: `absolute -bottom-6 right-1 flex items-center gap-1 text-amber-500 transition-opacity duration-300 ${canProceed ? "pointer-events-none opacity-0" : "opacity-100"}`,
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					...
					children: "选择版本类型和发布日期以获取对应的预设配置"
				})
			})]
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "page-content hide-scrollbar",
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "mx-auto flex w-full max-w-4xl flex-col gap-8",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(VersionSelector, {
						versions,
						selectedVersion: context.selectedVersionType,
						pendingVersion,
						onSelectVersion: selectVersion
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(LocalPresetList, {
						printerName: context.printer?.shortName || context.printer?.name || "--",
						selectedVersion: context.selectedVersionType,
						items: localItems,
						loading: localLoading
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(OnlinePresetList, { selectedVersion: context.selectedVersionType })
				]
			})
		})] });
	}
	//#endregion
	//#region src/renderer/react-app/pages/calibrate/CalibratePage.tsx
	function navigateToPage(page) {
		if (typeof document === "undefined") return;
		document.querySelector(`[data-page='${page}']`)?.click();
	}
	function invokeWindowHandler(name, ...args) {
		const fn = window[name];
		if (typeof fn === "function") return fn(...args);
		console.warn(`[MKP React Pages] calibrate handler unavailable: ${name}`);
	}
	function CalibratePage({ markReady }) {
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ReadySignal, {
			onReady: markReady,
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "page-header page-header-fixed-shell w-full",
				"data-react-page": "calibrate",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "page-header-top",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
						id: "title-page-calibrate",
						className: "text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100",
						children: "校准偏移"
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "flex items-center gap-3",
				...
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "page-header-sub" })]
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "page-content hide-scrollbar",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "max-w-4xl",
					children: [
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							className: "mb-5 rounded-xl border border-transparent bg-white p-5 card-shadow transition-colors dark:border-[#333] dark:bg-[#252526]",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "calibration-card-head",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: "calibration-card-title text-sm font-medium text-gray-700 dark:text-gray-300",
									children: "Z轴偏移校准"
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									className: "calibration-card-actions",
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
										id: "zDirectEditBtn",
										type: "button",
										...
										}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "打开校准模型" })]
									})]
								})]
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								id: "zCalibrationArea",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
										...
										children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
											className: "h-1 overflow-hidden rounded-full bg-gray-100 dark:bg-[#333]",
											children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "h-full w-full animate-pulse rounded-full theme-bg-solid" })
										})
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										id: "zPlaceholder",
										...
										children: [
											...
											/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
												id: "zCurrentOffsetSummary",
												className: "hidden mb-3 text-2xl font-black tracking-tight theme-text",
												children: ["当前预设的Z偏移值为：", /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
													id: "currentZOffsetDisplay",
													children: "--"
												})]
											}),
											/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
												className: "text-sm text-gray-400 dark:text-gray-500",
												children: "点击右上方打开校准模型按钮加载可视化校准网格"
											}),
											/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
												className: "text-sm text-gray-400 dark:text-gray-500",
												children: "点击后请耐心等待3mf打开..."
											})
										]
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
										...
										children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
											...
											children: [
												...
												/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
													className: "z-value-compare flex items-center justify-center gap-4 py-4",
													children: [
														/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
															className: "z-value-column text-center",
															children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
																className: "mb-1 text-xs text-gray-400",
																children: "原始值"
															}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
																id: "zOriginal",
																className: "z-value-number text-3xl font-semibold text-gray-400",
																children: "3.3"
															})]
														}),
														/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
															className: "z-value-arrow-column",
															children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
																id: "zBadge",
																className: "hidden mb-1 rounded-full px-2 py-0.5 text-xs font-medium theme-bg-soft theme-text",
																children: "-0.2"
															}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
																className: "z-value-arrow h-8 w-8 theme-text",
																fill: "none",
																stroke: "currentColor",
																viewBox: "0 0 24 24",
																children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
														...
														/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
															...
															children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
																...
																id: "zNewValue",
																className: "z-value-number text-3xl font-bold theme-text",
																children: "3.1"
															})]
														})
													]
								...
								children: [
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										...
										children: [
											...
											/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
												id: "xyCurrentOffsetSummary",
												className: "hidden mb-3 text-center text-xl font-black tracking-tight theme-text",
												children: [
													"当前预设的XY偏移值为：X ",
													/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
														id: "currentXOffsetDisplay",
														children: "--"
													}),
													" / Y ",
													/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
														id: "currentYOffsetDisplay",
														children: "--"
													})
												]
											}),
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										...
										className: "hidden",
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
											className: "xy-stage py-2",
											children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
												id: "xyYGrid",
												className: "xy-y-axis"
											}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
												className: "xy-main-column",
												children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
													className: "xy-summary-card",
													children: [
														/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
															className: "xy-summary-row",
															children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
																className: "xy-summary-label",
																children: "X"
															}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
																className: "xy-summary-values",
																children: [
																	/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
																		className: "xy-summary-value",
																		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
																			className: "xy-summary-caption",
																			children: "原始值"
																		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
																			id: "xyOriginalX",
																			className: "xy-summary-number",
																			children: "0.00"
																		})]
																	}),
																	/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
																		className: "xy-summary-arrow-wrap",
																		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
																			id: "xyBadgeX",
																			className: "hidden rounded-full px-2 py-0.5 text-xs font-medium theme-bg-soft theme-text",
																			children: "+0.00"
																		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("svg", {
																			className: "xy-summary-arrow h-7 w-7 theme-text",
																			fill: "none",
																			stroke: "currentColor",
																			viewBox: "0 0 24 24",
																			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", {
																	...
																	/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
																		...
																		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
																			...
																			id: "xyNewX",
																			className: "xy-summary-number theme-text",
																			children: "0.00"
																		})]
																	})
																]
														...
														/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "xy-summary-divider" }),
														/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
															className: "xy-summary-row",
															children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
																className: "xy-summary-label",
	...
	//#endregion
	//#region src/renderer/react-app/pages/params/ParamsPage.tsx
	function ParamsPage({ fallbackTarget, markReady }) {
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(LegacyShellBridge, {
			fallbackTarget,
			onReady: markReady
		});
	}
	//#endregion
	//#region src/renderer/react-app/pages/setting/SettingPage.tsx
	function SettingPage({ fallbackTarget, markReady }) {
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(LegacyShellBridge, {
			fallbackTarget,
			onReady: markReady
		});
	...
	//#endregion
	//#region src/renderer/react-app/entry/react-pages-entry.tsx
	var mountDownloadReactPage = createReactPageMount({
		pageName: "download",
		render: ({ markReady }) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ReadySignal, {
			onReady: markReady,
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(DownloadPage, {})
		})
	});
	var mountCalibrateReactPage = createReactPageMount({
		pageName: "calibrate",
		render: ({ markReady }) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CalibratePage, { markReady })
	});
	var mountParamsReactPage = createReactPageMount({
		pageName: "params",
		render: ({ fallbackTarget, markReady }) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ParamsPage, {
			fallbackTarget,
			markReady
		})
	});
	var mountSettingReactPage = createReactPageMount({
		pageName: "setting",
		render: ({ fallbackTarget, markReady }) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SettingPage, {
			fallbackTarget,
			markReady
		})
	});
	function mountRegisteredReactPages(rootDocument = document) {
		const pageMounts = [
			{
	...
	if (typeof window !== "undefined") window.MKPReactPagesBundle = {
	...
	//#endregion
	exports.mountCalibrateReactPage = mountCalibrateReactPage;
	exports.mountDownloadReactPage = mountDownloadReactPage;
	exports.mountParamsReactPage = mountParamsReactPage;
	exports.mountRegisteredReactPages = mountRegisteredReactPages;
	exports.mountSettingReactPage = mountSettingReactPage;
	return exports;
})({});
