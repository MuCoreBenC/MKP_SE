...
function getCurrentPresetContext() {
  const modernView = typeof window.__getDownloadContextView__ === 'function'
    ? window.__getDownloadContextView__()
    : null;
  const printerData = modernView?.printer || (typeof getPrinterObj === 'function' ? getPrinterObj(selectedPrinter) : null);
  const versionType = modernView?.selectedVersionType || selectedVersion;

  if (!printerData || !versionType) {
    return null;
  }
  return { printerData, versionType };
}
...
function saveCustomOrder(printerId = null, versionType = null) {
  const context = getCurrentPresetContext();
  const resolvedPrinterId = printerId || context?.printerData?.id || selectedPrinter;
  const resolvedVersionType = versionType || context?.versionType || selectedVersion;
  if (!resolvedPrinterId || !resolvedVersionType) return;
  ...
  localStorage.setItem(getCustomOrderKey(resolvedPrinterId, resolvedVersionType), JSON.stringify(newOrder));
}
...
function clearAppliedPresetSelection(printerData, versionType, fileNames) {
  ...
  const legacyActiveFile = localStorage.getItem(currentStorageKey);
  const activeFile = typeof window.resolveActivePresetFileName === 'function'
    ? window.resolveActivePresetFileName(window, legacyActiveFile)
    : legacyActiveFile;
...
async function renderPresetList(printerData, versionType) {
  ...
  const activeFileName = getResolvedActiveFileName(printerData.id, versionType);
  const hasApplied = typeof window.resolveDownloadAppliedState === 'function'
    ? window.resolveDownloadAppliedState(window, localData.map((item) => item.fileName), activeFileName)
    : localData.some((item) => item.fileName === activeFileName);
...
function renderListItems(container, releases, printerData, versionType, isLocal) {
  ...
  releases.forEach((release) => {
    const isApplied = isLocal && (typeof window.resolveDownloadAppliedState === 'function'
      ? window.resolveDownloadAppliedState(window, [release.fileName], activeFileName)
      : release.fileName === activeFileName);
...
function getDownloadContext(printerData) {
  const modernView = typeof window.__getDownloadContextView__ === 'function'
    ? window.__getDownloadContextView__()
    : null;

  return {
    printer: modernView?.printer || printerData || null,
    selectedVersionType: modernView?.selectedVersionType || selectedVersion || null
  };
}

function renderDownloadVersions(printerData) {
  const context = getDownloadContext(printerData);
  const resolvedPrinter = context.printer;
...
async function checkOnlineUpdates(btnElement) {
  ...
  try {
    const { printerData, versionType } = context;

    const cloudResult = await fetchCloudPresets(printerData.id, versionType);
    ...
    renderListItems(onlineList, cloudResult.data, printerData, versionType, false);
  } catch (error) {
...
function handleApplyLocal(releaseId, fileName, printerData, versionType = null, clickedBtn = null) {
  const context = getCurrentPresetContext();
  const resolvedVersionType = versionType || context?.versionType || selectedVersion;
  Logger.info('[O301] 应用本地预设', { fileName, versionType: resolvedVersionType });

  const currentKey = `${printerData.id}_${resolvedVersionType}`;
  const legacyActiveFile = localStorage.getItem(`mkp_current_script_${currentKey}`);
  const previousActiveFile = typeof window.resolveActivePresetFileName === 'function'
    ? window.resolveActivePresetFileName(window, legacyActiveFile)
    : legacyActiveFile;
  ...
  Array.from(localContainer.children).forEach((card) => {
    ...
    if (button) {
      if (isActiveCard) {
        const isReapply = previousActiveFile === fileName && button === clickedBtn;
...
async function handleDeleteLocal(releaseId, fileName, event, printerDataArg = null, versionTypeArg = null) {
  ...
  const context = getCurrentPresetContext();
  const printerData = printerDataArg || context?.printerData || (typeof getPrinterObj === 'function' ? getPrinterObj(selectedPrinter) : null);
  const versionType = versionTypeArg || context?.versionType || selectedVersion;
...
async function handleRenameLocal(target = presetContextMenuTarget) {
  ...
  await renderPresetList(resolvedPrinterData, resolvedVersionType);
}
