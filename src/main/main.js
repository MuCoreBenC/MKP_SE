...
const isAutoProcessMode = process.argv.includes('--auto-process');
...
function convertJsonToToml(jsonData) {
  const lines = [];
  
  // Toolhead section
  if (jsonData.toolhead) {
    lines.push('[toolhead]');
    if (jsonData.toolhead.speed_limit !== undefined) {
      lines.push(`speed_limit = ${jsonData.toolhead.speed_limit}`);
    }
    if (jsonData.toolhead.offset) {
      const o = jsonData.toolhead.offset;
      lines.push(`offset = { x = ${o.x}, y = ${o.y}, z = ${o.z}}`);
    }
    if (jsonData.toolhead.custom_mount_gcode) {
      lines.push('custom_mount_gcode = """');
      lines.push(jsonData.toolhead.custom_mount_gcode.trim());
      lines.push('"""');
    }
    if (jsonData.toolhead.custom_unmount_gcode) {
      lines.push('custom_unmount_gcode = """');
      lines.push(jsonData.toolhead.custom_unmount_gcode.trim());
      lines.push('"""');
    }
    lines.push('');
  }
  ...
  if (jsonData.wiping) {
    lines.push('[wiping]');
    const w = jsonData.wiping;
    if (w.have_wiping_components !== undefined) lines.push(`have_wiping_components = ${w.have_wiping_components}`);
    if (w.wiper_x !== undefined) lines.push(`wiper_x = ${w.wiper_x}`);
    if (w.wiper_y !== undefined) lines.push(`wiper_y = ${w.wiper_y}`);
    if (w.wipetower_speed !== undefined) lines.push(`wipetower_speed = ${w.wipetower_speed}`);
    if (w.nozzle_cooling_flag !== undefined) lines.push(`nozzle_cooling_flag = ${w.nozzle_cooling_flag}`);
    if (w.iron_apply_flag !== undefined) lines.push(`iron_apply_flag = ${w.iron_apply_flag}`);
    if (w.user_dry_time !== undefined) lines.push(`user_dry_time = ${w.user_dry_time}`);
    if (w.force_thick_bridge_flag !== undefined) lines.push(`force_thick_bridge_flag = ${w.force_thick_bridge_flag}`);
    if (w.support_extrusion_multiplier !== undefined) lines.push(`support_extrusion_multiplier = ${w.support_extrusion_multiplier}`);
  }
...
ipcMain.handle('init-default-presets', async () => {
  try {
    const { ensureMKPSEDirs, getMKPSEPresetsDir } = require('./release-config-ops');
    
    // 1. 确保 MKPSE 目录存在
    const { mkpseDir, presetsDir } = ensureMKPSEDirs();
    console.log("[系统初始化] 📁 创建 MKPSE 目录:", mkpseDir);
    console.log("[系统初始化] 📁 创建预设目录:", presetsDir);
    ...
    // 3. 从 cloud_data/presets 复制 TOML 文件到 MKPSE/presets 目录
    if (fs.existsSync(bundledPresetsPath)) {
      ...
      for (const file of files) {
        if (file.endsWith('.toml') || file.toLowerCase() === 'presets_manifest.json') {
          const sourceFile = path.join(bundledPresetsPath, file);
          const targetFile = path.join(presetsDir, file);
    ...
    // 4. 检测原始 MKPSupport 目录的 TOML 文件
    const originalMKPSupportPath = path.join(app.getPath('documents'), 'MKPSupport');
    if (fs.existsSync(originalMKPSupportPath)) {
      console.log(`[系统初始化] 🔍 检测到原始 MKPSupport 目录: ${originalMKPSupportPath}`);
      
      const originalFiles = fs.readdirSync(originalMKPSupportPath);
      for (const file of originalFiles) {
        if (file.endsWith('.toml')) {
          const sourceFile = path.join(originalMKPSupportPath, file);
          try {
            // 生成标准化的文件名
            const fileNameWithoutExt = path.basename(file, '.toml');
            const timestamp = new Date().toISOString().slice(0, 10).replace(/-/g, '');
            const newFileName = `${fileNameWithoutExt}_imported_${timestamp}.toml`;
            const targetFile = path.join(presetsDir, newFileName);
            
            // 复制文件
            fs.copyFileSync(sourceFile, targetFile);
            console.log(`[系统初始化] 📤 从原始 MKPSupport 导入预设: ${file} → ${newFileName}`);
            copiedCount++;
          } catch (error) {
            console.warn(`[系统初始化] ⚠️ 导入原始预设失败: ${file}`, error.message);
          }
        }
      }
    }
...
if (isAutoProcessMode) {
  ...
  app.whenReady().then(() => {
    try {
      console.log("[AutoProcess] 迷你进度条模式启动");
      
      // 解析命令行参数
      const tomlPath = process.argv[process.argv.indexOf('--Toml') + 1];
      const gcodePath = process.argv[process.argv.indexOf('--Gcode') + 1];
      
      if (!tomlPath || !gcodePath) {
        console.error("[AutoProcess] 参数缺失：需要 --Toml 和 --Gcode");
        app.quit();
        return;
      }
      
      if (!fs.existsSync(tomlPath)) {
        console.error("[AutoProcess] TOML 文件不存在: " + tomlPath);
        app.quit();
        return;
      }
      
      if (!fs.existsSync(gcodePath)) {
        console.error("[AutoProcess] G-code 文件不存在: " + gcodePath);
        app.quit();
        return;
      }
      ...
      progressWindow.webContents.once('did-finish-load', () => {
        const goExePath = path.join(__dirname, '../../bin/mkpsupport.exe');
        
        if (!fs.existsSync(goExePath)) {
          progressWindow.webContents.send('progress-error', 'Go 引擎未找到: ' + goExePath);
          setTimeout(() => app.quit(), 3000);
          return;
        }
        
        console.log("[AutoProcess] 启动 Go 引擎: " + goExePath);
        progressWindow.webContents.send('progress-update', { percent: 0, message: '正在启动处理引擎...' });
        
        const startTime = Date.now();
        const args = ['--Toml', tomlPath, '--Gcode', gcodePath, '--output-json'];
        const child = spawn(goExePath, args);
        ...
        child.stdout.on('data', (data) => {
          ...
          stdoutData = lines.pop(); // 保留不完整的行
          
          lines.forEach(line => {
            if (line.trim()) {
              try {
                const json = JSON.parse(line);
                console.log('[AutoProcess]', json);
                ...
                if (json.type === 'progress') {
                  ...
                  progressWindow.webContents.send('progress-log', {
                    level: json.level,
                    message: json.message
                  });
                }
        ...
        child.stderr.on('data', (data) => {
          ...
          console.error('[AutoProcess Error]', data.toString());
        });
        ...
        child.on('close', (code) => {
          ...
          if (code === 0) {
            console.log(`[AutoProcess] 处理完成，耗时: ${costTime}s`);
            progressWindow.webContents.send('progress-complete', {
              success: true,
              time: costTime
            });
            // 延迟退出，让用户看到完成状态
            ...
            const errorMsg = stderrData || `Go 进程退出码: ${code}`;
            console.error("[AutoProcess] 处理失败: " + errorMsg);
            progressWindow.webContents.send('progress-complete', {
              success: false,
              error: errorMsg
            });
        ...
        child.on('error', (err) => {
          console.error("[AutoProcess] 启动失败: " + err.message);
          progressWindow.webContents.send('progress-error', err.message);
          setTimeout(() => app.quit(), 3000);
        });
      });
      
    } catch (error) {
      console.error("[AutoProcess] 初始化失败: " + error.message);
      app.quit();
    }
  });
  ...
  app.whenReady().then(() => {
    try {
      ...
      if (!fs.existsSync(goExePath)) {
        throw new Error('Go executable not found at: ' + goExePath);
      }
      
      // Read and convert JSON preset to TOML
      const jsonContent = fs.readFileSync(jsonPath, 'utf8');
      const jsonData = JSON.parse(jsonContent);
      const tomlContent = convertJsonToToml(jsonData);
      
      // Write temporary TOML file
      const tempTomlPath = path.join(app.getPath('temp'), `mkp_preset_${Date.now()}.toml`);
      fs.writeFileSync(tempTomlPath, tomlContent, 'utf-8');
      
      const outputPath = gcodePath.replace('.gcode', '_processed.gcode');
      
      // Call Go executable
      const args = ['--Toml', tempTomlPath, '--Gcode', gcodePath, '--output-json'];
      const child = spawn(goExePath, args);
      ...
      child.on('close', (code) => {
        // Clean up temporary TOML file
        try { fs.unlinkSync(tempTomlPath); } catch (e) {}
        ...
        if (code === 0) {
          ...
          const errorMsg = stderrData || `Go process exited with code ${code}`;
          console.error("[E604] CLI exec err: " + errorMsg);
          if (Notification.isSupported()) {
            new Notification({ title: 'MKP 处理失败', body: `❌ ${errorMsg}` }).show();
          }
      ...
      child.on('error', (err) => {
        ...
        console.error("[E604] CLI exec err: " + err.message);
        if (Notification.isSupported()) {
          new Notification({ title: 'MKP 处理失败', body: `❌ ${err.message}` }).show();
        }
        setTimeout(() => app.quit(), 5000);
      });
    } catch (error) {
  ...
  ipcMain.handle('get-exe-path', () => {
    // 返回 Go 引擎的路径，而不是 Electron 的路径
    const goEnginePath = path.join(__dirname, '../../bin/mkpsupport.exe');
    if (fs.existsSync(goEnginePath)) {
      return goEnginePath;
    }
    // 如果找不到 Go 引擎，回退到原来的 Electron 路径
...
ipcMain.handle('overwrite-preset', async (event, filePath, data) => {
  try {
    ...
    if (filePath.toLowerCase().endsWith('.toml')) {
      const tomlContent = convertJsonToToml(data);
...
// ==========================================
ipcMain.handle('process-gcode-with-go', async (event, { gcodePath, tomlPath }) => {
  const goExePath = path.join(__dirname, '../../bin/mkpsupport.exe');
  ...
  if (!fs.existsSync(gcodePath)) {
    return { success: false, error: 'G-code file not found' };
  }
  
  if (!fs.existsSync(tomlPath)) {
    return { success: false, error: 'TOML config file not found' };
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', (data) => {
      ...
      lines.forEach(line => {
        if (line.trim()) {
          try {
            const json = JSON.parse(line);
            // Forward progress message to renderer
            event.sender.send('go-progress', json);
          } catch (e) {
            // ignore non-JSON lines
          }
        }
    ...
    child.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        reject(new Error(stderrData || `Go process exited with code ${code}`));
      }
    });
