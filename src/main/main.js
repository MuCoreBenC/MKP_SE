...
const { resolveWindowEntryTarget } = require('./window-entry.cjs');
...
function getRendererEntryPath() {
  return path.join(getAppContentRootPath(), 'dist', 'index.html');
}

function getReleaseCenterEntryPath() {
  return path.join(getAppContentRootPath(), 'dist', 'release_center.html');
}
...
if (isCliMode) {
  ...
  function createReleaseCenterWindow() {
    ...
    const releaseCenterEntryPath = getReleaseCenterEntryPath();
    const legacyReleaseCenterEntryPath = path.join(__dirname, '../renderer/release_center.html');
    const releaseWindowTarget = resolveWindowEntryTarget({
      devServerUrl,
      distHtmlPath: releaseCenterEntryPath,
      legacyHtmlPath: legacyReleaseCenterEntryPath,
      pageName: 'release_center.html',
      fsExistsSync: fs.existsSync
    });

    if (releaseWindowTarget.kind === 'url') {
      releaseWindow.loadURL(releaseWindowTarget.target);
      return;
    }

    releaseWindow.loadFile(releaseWindowTarget.target);
  }
  ...
  mainWindow.once('ready-to-show', () => {
    ...
    const legacyRendererEntryPath = path.join(__dirname, '../renderer/index.html');
    const mainWindowTarget = resolveWindowEntryTarget({
      devServerUrl,
      distHtmlPath: rendererEntryPath,
      legacyHtmlPath: legacyRendererEntryPath,
      pageName: 'index.html',
      fsExistsSync: fs.existsSync
    });

    if (mainWindowTarget.kind === 'url') {
      mainWindow.loadURL(mainWindowTarget.target);
      return;
    }

    mainWindow.loadFile(mainWindowTarget.target);
  }
