    ...
    <div id="page-download" className="page flex-1 flex flex-col h-full bg-gray-50/50 dark:bg-[#1A1A1A]">
      ...
      <div className="max-w-5xl mx-auto w-full p-4 sm:p-8 flex-1 flex flex-col min-h-0 overflow-y-auto hide-scrollbar pb-24">
        ...
        <section className="mb-8 shrink-0 animate-scale-in" style={{ animationDelay: '50ms' }}>
          ...
          {activeFile ? (
            <div className="bg-white dark:bg-[#252526] rounded-2xl border-2 theme-border shadow-[0_8px_30px_rgba(var(--primary-rgb),0.08)] p-6 relative overflow-hidden transition-all">
              {/* 右上角精美徽标 */}
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mt-2">
                <div>
                  <p className="text-xs theme-text font-bold mb-2 flex items-center uppercase tracking-wider">
                    <span className="w-2 h-2 rounded-full theme-bg-solid mr-2 animate-pulse shadow-sm"></span> 
                    正在应用中
                  </p>
                  <h3 className="font-bold text-lg sm:text-xl text-gray-900 dark:text-gray-100 font-mono tracking-tight">
                    {activeFile}
                  </h3>
                </div>
                <button 
                  onClick={handleDeleteLocal}
                  className="text-gray-400 hover:text-red-500 bg-gray-50 hover:bg-red-50 dark:bg-[#1E1E1E] dark:hover:bg-red-900/30 transition-all p-2.5 rounded-xl shadow-sm self-start" 
                  title="删除本地文件"
                >
            ...
            <div className="bg-white dark:bg-[#252526] rounded-2xl border border-dashed border-gray-300 dark:border-[#444] p-10 flex flex-col items-center justify-center text-center shadow-sm">
        ...
        {/* ==========================================
            第二部分：在线资源库 (现代 SaaS 样式折叠面板)
            ========================================== */}
        <section className={`bg-white dark:bg-[#252526] rounded-2xl border border-gray-200/80 dark:border-[#333] shadow-sm flex flex-col transition-all duration-300 shrink-0 ${isDownloadExpanded ? 'ring-4 ring-gray-50 dark:ring-white/5' : ''}`}>
          ...
          >
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-gray-50 dark:bg-[#1E1E1E] rounded-2xl border border-gray-100 dark:border-[#444] flex items-center justify-center shadow-inner transition-colors">
                <Icon name="download" className={`w-6 h-6 ${isDownloadExpanded ? 'theme-text' : 'text-gray-500'}`} />
              </div>
              <div>
                <h2 className="text-base sm:text-lg font-bold text-gray-900 dark:text-gray-100">在线参数资源库</h2>
                <p className="text-xs sm:text-sm text-gray-500 mt-1">
                  {isDownloadExpanded ? '探索系统内置与社区贡献的最佳参数' : '点击展开以获取更多打印件及工艺预设'}
                </p>
              </div>
            </div>
            <div className={`w-10 h-10 flex items-center justify-center rounded-full bg-gray-50 dark:bg-[#1E1E1E] transition-all duration-300 ${isDownloadExpanded ? 'rotate-180 bg-blue-50 dark:bg-blue-900/20 theme-text' : 'text-gray-400'}`}>
          ...
          <div className={`collapse-wrapper ${isDownloadExpanded ? 'is-open is-expanded' : ''}`}>
            <div className="collapse-inner flex flex-col">
              ...
              <div className="px-5 pt-4 pb-2 border-b border-gray-100 dark:border-[#333] bg-white dark:bg-[#252526]">
                <div className="flex p-1 bg-gray-100 dark:bg-[#1E1E1E] rounded-xl overflow-x-auto hide-scrollbar">
                  {[
                    ...
                    <button 
                      key={tab.id}
                      className={`flex-1 min-w-max px-4 py-2.5 text-sm font-bold rounded-lg transition-all ${activeTab === tab.id ? 'bg-white dark:bg-[#333] text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
              ...
              <div className="p-5 sm:p-6">
                ...
                {/* === TAB 3: BBS 社区工艺预设 (重点重构区) === */}
                {activeTab === 'bbs' && (
                  <div className="flex flex-col animate-scale-in">
                    
                    {/* 过滤器区域：柔和的卡片包围 */}
                    <div className="bg-gray-50 dark:bg-[#1E1E1E] p-4 sm:p-5 rounded-2xl border border-gray-100 dark:border-[#333] mb-6 flex flex-col gap-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {['0.2', '0.4', '0.6'].map(nozzle => (
                            <button 
                              ...
                              className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${currentNozzle === nozzle ? 'theme-bg-solid shadow-md' : 'bg-white dark:bg-[#252526] border border-gray-200 dark:border-[#444] text-gray-600 dark:text-gray-300 hover:border-gray-300'}`}
                            >{nozzle} mm</button>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                        <div className="flex items-center gap-2 w-20">
                          <span className="text-sm font-bold text-gray-700 dark:text-gray-300">包含层高</span>
                        </div>
                          ...
                          {LAYER_DATA[currentNozzle].map(layer => {
                            ...
                            return (
                              <button 
                                ...
                                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${isActive ? 'bg-emerald-500 text-white shadow-md border-transparent' : 'bg-white dark:bg-[#252526] border border-gray-200 dark:border-[#444] text-gray-600 dark:text-gray-300 hover:border-gray-300'}`}
                              >{layer}</button>
                            );
                          })}
                        </div>
                    ...
                    <div className="flex items-center justify-between mb-4 pl-1">
                      <label className="flex items-center gap-3 cursor-pointer group">
                        <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${isAllSelected ? 'theme-bg-solid border-transparent' : 'border-gray-300 dark:border-gray-500 group-hover:border-blue-400'}`}>
                      ...
                      {selectedBbsFiles.size > 0 && (
                        <button className="theme-btn-solid px-5 py-2 rounded-xl text-sm font-bold shadow-md active:scale-95 transition-all flex items-center gap-2">
                    ...
                    {/* 呼吸感卡片列表 */}
                    {bbsFiles.length === 0 ? (
                      <div className="py-12 text-center text-gray-400 font-medium bg-gray-50/50 dark:bg-[#1E1E1E] rounded-2xl border border-dashed border-gray-200 dark:border-[#333]">
                        未找到符合条件的预设文件
                      </div>
                    ) : (
                      <div className="flex flex-col gap-3">
                        {bbsFiles.map(file => {
                          ...
                          const cardClass = isSelected 
                            ? 'theme-bg-soft border-transparent ring-2 ring-[rgba(var(--primary-rgb),0.4)] shadow-sm'
                            : 'bg-white dark:bg-[#252526] border-gray-200 dark:border-[#444] hover:border-gray-300 dark:hover:border-gray-500 hover:shadow-sm';
                          ...
                          return (
                            ...
                            >
                              <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-all ${isSelected ? 'theme-bg-solid border-transparent' : 'border-gray-300 dark:border-gray-500 group-hover:border-blue-400'}`}>
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="font-mono text-sm sm:text-base font-bold text-gray-900 dark:text-gray-100 truncate mb-1.5">
                                  {file.filename}
                                </div>
                                <div className="flex flex-wrap items-center gap-2">
                                  <span className="bg-gray-100 dark:bg-[#333] text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded-md text-xs font-medium border border-gray-200/50 dark:border-transparent">喷嘴 {file.nozzle}</span>
                                  <span className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded-md text-xs font-bold tracking-wide">层高 {file.layer}</span>
                                  <span className="text-xs text-gray-400 font-medium ml-2">作者: {file.author}</span>
                                </div>
                              </div>

                              <button 
                                className="opacity-0 group-hover:opacity-100 px-4 py-2 bg-white dark:bg-[#333] border border-gray-200 dark:border-[#555] text-gray-700 dark:text-gray-200 hover:theme-text hover:theme-border rounded-xl text-xs font-bold transition-all shadow-sm shrink-0"
