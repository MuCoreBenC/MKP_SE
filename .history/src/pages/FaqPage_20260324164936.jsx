...
// ==========================================
// 💡 1. 核心高亮引擎组件 (极简且强大)
// 它会安全地将匹配到的文本切开，并用 Tailwind 的高亮颜色包裹
// ==========================================
const HighlightText = ({ text, query }) => {
  if (!query.trim()) return <>{text}</>;

  // 使用正则忽略大小写(i)并全局匹配(g)，同时保留匹配到的词
  ...
  return (
    <>
      {parts.map((part, index) =>
        regex.test(part) ? (
          // 匹配到的部分：套上带微透明度的黄色高亮背景（完美适配暗黑模式）
          ...
          <div id="faqList" className="space-y-3">
            {filteredFaqs.length > 0 ? (
              filteredFaqs.map(item => (
                // 把 searchQuery 传给子组件，让它知道该高亮什么词！
